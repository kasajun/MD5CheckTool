...
static void sha512_noasm_block_data_order(SHA512_CTX* ctx, const void* in, size_t num);
...
int SHA512_NOASM_Final(unsigned char* md, SHA512_CTX* c)
{
	...
	if (n > (sizeof(c->u) - 16))
	{
		...
		n = 0;
		sha512_noasm_block_data_order(c, p, 1);
	}
...
int SHA512_NOASM_Update(SHA512_CTX* c, const void* _data, size_t len)
{
	SHA_LONG64 l;
...
#endif
		{
			sha512_noasm_block_data_order(c, data, len / sizeof(c->u));
...
#endif
	sha512_noasm_block_data_order(c, data, 1);
}
...
unsigned char* SHA384_NOASM(const unsigned char* d, size_t n, unsigned char* md)
{
	...
	SHA384_NOASM_Init(&c);
	SHA512_NOASM_Update(&c, d, n);
	SHA512_NOASM_Final(md, &c);
