...
#define HASH_UPDATE				SHA256_NOASM_Update
#define HASH_TRANSFORM			SHA256_NOASM_Transform
#define HASH_FINAL				SHA256_NOASM_Final
#define HASH_BLOCK_DATA_ORDER	sha256_noasm_block_data_order

static void sha256_noasm_block_data_order(SHA256_CTX* ctx, const void* in, size_t num);
...
int SHA256_192_NOASM_Init(SHA256_CTX* c)
{
	memset(c, 0, sizeof(*c));
	c->h[0] = 0x6a09e667;
	c->h[1] = 0xbb67ae85;
...
unsigned char* SHA224_NOASM(const unsigned char* d, size_t n, unsigned char* md)
{
	...
	SHA224_NOASM_Init(&c);
	SHA256_NOASM_Update(&c, d, n);
	SHA256_NOASM_Final(md, &c);
